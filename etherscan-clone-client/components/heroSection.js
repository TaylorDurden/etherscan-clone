...
const getEthPriceApi = 'http://localhost:5001/getethprice';
const getBlockInfoApi = 'http://localhost:5001/getblockinfo';
...
export default function HeroSection() {
  ...
  useEffect(() => {
    const getEthPrice = async () => {
      const response = await axios.get(getEthPriceApi, {});
      setEthPrice(response.data.usdPrice);
    };

    const getBlockInfo = async () => {
      const response = await axios.get(getBlockInfoApi, {});
      console.log('rrr', response);
      const previousBlockInfo = response.data.previousBlockInfo;
      const blockArray = [
        ...previousBlockInfo.filter((value, index) => index !== 0),
      ];

      const transactions = [previousBlockInfo[0].transactions];

      console.log('transactions', transactions[0]);
      setTotalTransactions(previousBlockInfo[1].totalTransactions);
      setLatestBlock(response.data.latestBlock);
      setBlockResult(blockArray);
      setTransactionsResult(previousBlockInfo[0].transactions);
    };
